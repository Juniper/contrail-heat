...
class HeatServiceTemplate(ContrailResource):
    ...
    properties_schema = {
        ...
        SERVICE_MODE: properties.Schema(
            ...
            _('Service Mode'),
            constraints=[
                constraints.AllowedValues(['transparent', 'in-network', 'in-network-nat']),
            ],
            update_allowed=False,
            required=True,
        ),
	SERVICE_TYPE: properties.Schema(
            ...
            _('Service Type'),
            constraints=[
                constraints.AllowedValues(['firewall', 'analyzer', 'source-nat', 'loadbalancer']),
            ],
            update_allowed=False,
            required=True,
        ),
            ...
            schema=properties.Schema(
                ...
                schema={
                    SERVICE_INTERFACE_TYPE: properties.Schema(
                        ...
                        required=True,
            		constraints=[
                		constraints.AllowedValues(['management', 'left', 'right', 'other']),
            		],
                    ),
	...
	SERVICE_VIRT_TYPE: properties.Schema(
            ...
            constraints=[
		constraints.AllowedValues(['virtual-machine', 'network-namespace']),
            ],
            update_allowed=False
        ),
    ...
    attributes_schema = {
        ...
        "service_instaces": _("Service Instances launched with this service template."),
    ...
    def handle_create(self):
        domain = self.vnc_lib().domain_read(fq_name=[self.properties[self.DOMAIN]])
        st_obj = ServiceTemplate(name=self.properties[self.NAME], parent_obj=domain)
        ...
        svc_properties.set_service_scaling(self.properties[self.SERVICE_SCALING])
        ...
        svc_properties.set_service_virtualization_type(self.properties[self.SERVICE_VIRT_TYPE])
        # set interface list
        for itf in self.properties[self.INTERFACE_TYPE]:
            if_type = ServiceTemplateInterfaceType(shared_ip=itf[self.SHARED_IP])
            if_type.set_service_interface_type(itf[self.SERVICE_INTERFACE_TYPE])
